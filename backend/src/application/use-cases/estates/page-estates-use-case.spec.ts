...
describe('Page Estate UseCase', () => {
  ...
  it('should be able to page Estates', async () => {
    ...
    expect(response.nodes?.[0]).toBeInstanceOf(Estate);

    expect(response.totalCount).toBe(1);
    expect(response.nodes).toEqual<Estate[]>([estate]);
  });

  it('should be able to filter Estates in pager', async () => {
    const estate = await createEstateUseCase.handle({
      name: 'Estate Example',
      description: 'Description example',
      address: {
    ...
    const estateNotIncluded = await createEstateUseCase.handle({
      name: 'Estate Not Included',
      description: 'Description example',
      address: {
        city: 'City example',
        number: 123,
    ...
    expect(response.totalCount).toBe(1);
    expect(response.nodes).toEqual<Estate[]>(expect.arrayContaining([estate]));

    expect(response.nodes).not.toEqual<Estate[]>(
      expect.arrayContaining([estateNotIncluded]),
    );
  });
});
